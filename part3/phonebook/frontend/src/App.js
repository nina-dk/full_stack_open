...
const App = () => {
    ...
    if (nameExists(newName)) {
      ...
      contactService.update(updatedContact)
                    .then(_ => {
                      ...
                      createNotification(`Updated ${contact.name}'s number.`, "success");
                      resetFormFields();
                    })
                    .catch(err => {
                      console.log(err);
                      ...
                      if (err.response.status === 404) {
                        createNotification(`${contact.name} has been removed from your contacts.`, "error");
      ...
      contactService.create(contactData)
                    .then(newContact => {
                      ...
                      createNotification(`Added ${contactData.name} to the phonebook.`, "success");
                    })
                    .catch(err => {
                      console.log(err);
                      createNotification(err.response.data.error, "error");
  ...
  const deleteContact = id => {
    ...
    if (!contact) {
      createNotification(`Contact with id ${id} has already been deleted.`, "error");
    ...
    contactService.deleteContact(id)
                  .then(_ => {
                    setPersons(persons.filter(person => person.id !== id));
                    createNotification(`${contact.name} deleted successfully.`, "success");
                  })
                  .catch(err => console.log(err));
  }
